                    ...
                    for (let user of participants) {
                       let pp = './src/welcome.jpg'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || '@user Yah,si Beban Masuk Grup').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
                                (chat.sBye || this.bye || conn.bye || 'Sip @user , Beban Berkurang 1'))
                                this.sendButtonLoc(id, await(await fetch(pp)).buffer(), text, '', "𝑩𝑰𝑬𝑵𝑽𝑬𝑵𝑰𝑫𝑶", "huuu", null)
                                }
                    }
                }
                break
